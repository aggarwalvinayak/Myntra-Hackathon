...
def load_data_numpy(df):
    ...
    # print("asdfffff",label_array)

    x1 = df['x1'].as_matrix().reshape(-1, 1)
    y1 = df['y1'].as_matrix().reshape(-1, 1)
    x2 = df['x2'].as_matrix().reshape(-1, 1)
    y2 = df['y2'].as_matrix().reshape(-1, 1)
