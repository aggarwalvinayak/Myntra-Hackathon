...
class Train:
    ...
    def top_k_error(self, predictions, labels, k):
        ...
        in_top1 = tf.to_float(tf.nn.in_top_k(targets=labels,predictions=predictions, k=1))
    ...
    def train(self):
        train_df = prepare_df(FLAGS.train_path, usecols=['image_path', 'category', 'x1', 'y1', 'x2', 'y2'])
    ...
    def test(self):
        self.test_image_placeholder = tf.placeholder(dtype=tf.float32, shape=[1, IMG_ROWS,
                                                                              IMG_COLS, 3])
        self.test_label_placeholder = tf.placeholder(dtype=tf.int32, shape=[1])

        ##########################
        # Build test graph
        logits, global_pool,_ = inference(self.test_image_placeholder, n=FLAGS.num_residual_blocks, reuse=False,
                                              keep_prob_placeholder=self.dropout_prob_placeholder)
        predictions = tf.nn.softmax(logits)
        ...
        prediction_np = np.array([]).reshape(-1, 51)
        fc_np = np.array([]).reshape(-1, 4)
        print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        print(len(test_df))
        ...
        for step in range(len(test_df) // TEST_BATCH_SIZE):
            ...
            df_batch = test_df.iloc[step*1 : (step+1)*1, :]
            test_batch, test_label, _= load_data_numpy(df_batch)
        ...
        np.savetxt("outpred.csv", prediction_np, delimiter=",")
        # print(prediction_np)
        np.save(FLAGS.fc_path, fc_np)
...
def download_img():
    ...
    config = {
      "apiKey": "AIzaSyAIOVX-VGWmJqfNsez2fh1au5covqZC6Nc",
      "authDomain": "my-bong.firebaseapp.com",
      "databaseURL": "https://my-bong.firebaseio.com",
      "storageBucket": "my-bong.appspot.com"
    }
    firebase=pyrebase.initialize_app(config)
    # bucket = storage.Client()
    # storage_client = storage.Client()
    # bucket = storage_client.get_bucket('my-bong.appspot.com')
    # blob = bucket.blob('abbc.jpg')

    # dat=blob.download_as_string()
    # file1 = open("imgg.jpeg","w")#write mode 
    # file1.write(dat) 
    # file1.close()
    ...
    # cred = credentials.Certificate('./My-Bong-5ed48b20aefb.json')
    # bucket = storage.Client()
    # bucket = bucket.get_bucket('my-bong.appspot.com')
    db = firebase.database()
    
    url=db.child("user_uploaded").get().val()
    urllib.request.urlretrieve(url, 'imgg.jpeg')

    # data = {"1":aa,"2":bb}
    # storage=firebase.storage()
    # storage.child('user_uploaded/img.jpeg').download("imgg.jpeg")
    # storage_client = storage.Client()
    # bucket = storage_client.get_bucket(bucket_name)
    # blob = bucket.blob(source_blob_name)

    # blob.download_to_filename(destination_file_name)
    # blob = bucket.blob('user_uploaded/img.jpeg')
    # blob.download_to_filename(filename="imgg.jpeg")
 
    # url=blob.generate_signed_url(expiration=timedelta(100)  )

def upload_var(aa,bb):
    import sys
    import requests
    import firebase_admin
    from firebase_admin import credentials
    ...
    cred = credentials.Certificate('./My-Bong-5ed48b20aefb.json')
    db = firebase.database()
    db.child("category_no")
    data = {"1":aa,"2":bb}

    # db.child("users3").child("qwer").push(data)
    # print(1)
    db.set(data)
    # bucket = storage.Client()
...
def fn(path):
    ...
    pred = genfromtxt(path, delimiter=',')
